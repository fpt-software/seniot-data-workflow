				...
				if (this.overwriteFile === "true") {
					...
					fs.writeFile(filename, data, "utf8", function(err) {
						...
						if (err) {
							...
							node.log(RED._("file.status.wrotefile", {
								file : filename
							}));
						}
					});
					...
					fs.unlink(filename, function(err) {
						if (err) {
							node.error(RED._("file.errors.deletefail", {
								error : err.toString()
							}), msg);
						} else if (RED.settings.verbose) {
							node.log(RED._("file.status.deletedfile", {
								file : filename
							}));
						}
					});
					...
					fs.appendFile(filename, data, "binary", function(err) {
						...
						if (err) {
							if ((err.code === "ENOENT") && node.createDir) {
								fs.ensureFile(filename, function(err) {
									if (err) {
										...
										fs.appendFile(filename, data, "binary", function(err) {
											if (err) {
												node.error(RED._("file.errors.appendfail", {
													error : err.toString()
												}), msg);
											}
										});
							...
							node.log(RED._("file.status.appendedfile", {
								file : filename
							}));
						}
					});
